...
event__part--1"]'
    ...
    def __init__(self, rsl_url, rsl_fPath, fix_fPath, nm_table,
                 surfix, lines=10):
        ...
        self.surfix = surfix
        self.lines = lines  # number of lines to crawl during Scraping
        self.options = webdriver.FirefoxOptions()
        self.options.headless = True
        self.driver = webdriver.Firefox(options=self.options)
        self.driver.implicitly_wait(30)
    ...
    def parse_item(self):
        """collect date, time, teams and goals of each event,
            ...
            and save one copy in csv file inside rsl_fPath.
        """
        ...
        try:
            ...
            if len(match_play) == 7 or len(match_play) == 8:
                ...
                match_play = match_play[:10]
                # in the latter the text is JOURNÉE
                # exple = JOURNÉE 13
            for event in event_match[:self.lines]:
                item = {}
                item['mp'] = match_play
...
/2022_23/results/super_lig.csv")
    ...
    fixSpider = FixtureSpider(fix_url, fix_fPath).parse_item()
